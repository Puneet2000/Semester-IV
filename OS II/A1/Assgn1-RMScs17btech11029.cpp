...
public:
	...
	Process(int pid,int process_time,int period,int arrival_time){
		...
		this->interuppted = false; // wheter it was preempted
...
Process schedule(map<int,Process>* ready_queue){ // scheduler to take out highest priority process and return it
	...
	for (itr = ready_queue->begin(); itr != ready_queue->end(); itr++) {  // less period means high priority
        if ((itr->second).period < min){
        	min_itr = itr;
        	min = (itr->second).period;
        }
    }
...
int main(){
	...
	while(!processes.empty()){ // do till all process are not considered
		...
		if (running.period<prev_running.period){ // compare the selected process from prev running process
