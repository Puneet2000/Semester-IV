...
float mu_cs , mu_rem ;
int read_count = 0;
FILE* f=fopen("RW-log.txt","w"); // file pointer
...
double reader_avg_time = 0 , writer_avg_time = 0 , reader_max_time =0 , writer_max_time = 0; // metrics
...
void *reader(void *param){
	...
	for(int i=0;i<k_r;i++){
		...
		sem_wait(&mutex);
		read_count++;
		if (read_count == 1) // block all writers
			sem_wait(&rw_mutex);
		sem_post(&mutex);
...
int main(){
	...
	fprintf(f2,"Average time in standard reader-wrters is %lf miliseconds\n" , (writer_avg_time + reader_avg_time)/(n_r*k_r*1e3 + n_w*k_w*1e3));
